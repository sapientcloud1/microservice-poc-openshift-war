...
class Haproxy
	...
	@log.error("Before for sapedomain33333")
      @status_urls = []
	...
	@log.error("Before for sapedomain444444444")
        @previous_stats = []
        ...
        self.refresh(false)
        @log.info("Starting JJ haproxy_ctld")
        self.print_gear_stats
    end

    def get_remote_sessions_count(status_url)
	@log.error("Before for sapedomain666666666")
	...
	@log.error("Before for sapedomain1111111")
	...
	@log.error("Before for sapedomain777777777777")
      num = 0
      count = 1
	...
	@log.error("Before for sapedomain888888888")
        @last_scale_up_time = Time.now
	...
	@log.error("Before for sapedomain9999999")
	...
	@log.error("Before for sapedomain0000")
      begin
	...
	@log.error("Before for sapedomain START")
        ...
        if lsets == 0 || lsets > FLAP_PROTECTION_TIME_SECONDS
          ...
          # pseudo code below:
          #
		  @log.error("Before for sapedomain")
          for gear_dns in gear_list
               ...
               self.add_gear if mem_usage >= 10000000
			    @log.error("sapedomain")
           end
		     @log.debug("after sapedomain")
